...
CONFIG = {
  ...
  "display": "DOCX Unit Symbol Consistency",
  "short": "Normalize DOCX unit symbols.",
  "prompt": "Check this DOCX for inconsistent unit symbols and apply tracked changes.",
  "desc": "Use when checking or fixing DOCX unit and symbol consistency, especially Angstrom notation, malformed units, and spacing around scientific units.",
  "mode_desc": "unit and symbol consistency, such as Angstrom notation and malformed unit expressions.",
  "mode": "replacements",
  "replacements": [
    {
      "old": "A˚",
      "new": "Å",
      "message": "Use standard Angstrom notation."
    },
    {
      "old": "A°",
      "new": "Å",
  ...
  "script": "check_unit_symbol_consistency.py",
  ...
  "display_name": "DOCX Unit Symbol Consistency"
}
        ...
        if mode == 'replacements':
            ...
            for item in CONFIG.get('regex_replacements',[]):
                for m in re.finditer(item['pattern'], text):
                    old=m.group(0); new=item['replacement'].replace('{match}', old)
